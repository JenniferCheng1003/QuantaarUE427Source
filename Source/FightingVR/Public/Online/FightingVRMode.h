...
#include "FightingVRMode.generated.h"

class AFightingVRAIController;
class AFightingVRPlayerState;
class AFightingVRPickup;
class FUniqueNetId;

UCLASS(config=Game)
class AFightingVRMode : public AGameMode
{
	...
	virtual bool CanDealDamage(AFightingVRPlayerState* DamageInstigator, AFightingVRPlayerState* DamagedPlayer) const;
	...
	/** Create a bot */
	AFightingVRAIController* CreateBot(int32 BotNum);	
...
protected:
	...
	UPROPERTY()
	TArray<AFightingVRAIController*> BotControllers;

	UPROPERTY(config)
	TSubclassOf<AFightingVRPlayerController> PlatformPlayerControllerClass;
	...
	virtual void InitBot(AFightingVRAIController* AIC, int32 BotNum);

	/** check who won */
	virtual void DetermineMatchWinner();

	/** check if PlayerState is a winner */
	virtual bool IsWinner(AFightingVRPlayerState* PlayerState) const;
...
public:	
	...
	UPROPERTY()
	TArray<AFightingVRPickup*> LevelPickups;
