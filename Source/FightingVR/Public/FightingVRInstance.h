...
#include "FightingVRInstance.generated.h"

class FVariantData;
class FFightingVRMainMenu;
class FFightingVRWelcomeMenu;
class FFightingVRMessageMenu;
class AFightingVRSession;
...
class FFightingVRPendingMessage
{
public:
...
class FFightingVRPendingInvite
{
public:
	FFightingVRPendingInvite() : ControllerId(-1), UserId(nullptr), bPrivilegesCheckedAndAllowed(false) {}
...
class SFightingVRWaitDialog : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SFightingVRWaitDialog)
	{}
...
UCLASS(config=Game)
class UFightingVRInstance : public UGameInstance
{
public:
...
public:
	...
	AFightingVRSession* GetGameSession() const;
	...
	void SetPendingInvite(const FFightingVRPendingInvite& InPendingInvite);
...
private:
	...
	FFightingVRPendingMessage PendingMessage;
	...
	/** Main menu UI */
	TSharedPtr<FFightingVRMainMenu> MainMenuUI;

	/** Message menu (Shown in the even of errors - unable to connect etc) */
	TSharedPtr<FFightingVRMessageMenu> MessageMenuUI;

	/** Welcome menu UI (for consoles) */
	TSharedPtr<FFightingVRWelcomeMenu> WelcomeMenuUI;

	/** Dialog widget to show non-interactive waiting messages for network timeouts and such. */
	TSharedPtr<SFightingVRWaitDialog> WaitMessageWidget;
