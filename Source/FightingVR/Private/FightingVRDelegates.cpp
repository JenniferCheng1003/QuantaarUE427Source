...
struct FFightingVRGlobalDelegateInit
{
	FFightingVRGlobalDelegateInit()
	{
		FPakPlatformFile::FPakSigningFailureHandlerData& HandlerData = FPakPlatformFile::GetPakSigningFailureHandlerData();
		{
			...
			HandlerData.ChunkSignatureCheckFailedDelegate.AddStatic(FFightingVRGlobalDelegateInit::HandlePakChunkSignatureCheckFailed);
			HandlerData.MasterSignatureTableCheckFailedDelegate.AddStatic(FFightingVRGlobalDelegateInit::HandlePakMasterSignatureTableCheckFailure);
		}
	...
	{
		UE_LOG(LogFightingVR, Fatal, TEXT("Pak chunk signature check failed!"));
	}

	static void HandlePakMasterSignatureTableCheckFailure(const FString& InPakFilename)
	{
		UE_LOG(LogFightingVR, Fatal, TEXT("Pak master signature table check failed for pak '%s'"), *InPakFilename);
	}
	...
	if (URL == TEXT("/index.html?scoreboard"))
	{
		...
		if (GameEngine)
		{
			...
			{			
				...
				{
					// get the shoter game
					AFightingVRState* const GameState = Player->PlayerController->GetWorld()->GetGameState<AFightingVRState>();
...
void InitializeFightingVRDelegates()
{
