...
/**
*	
*	===================== FightingVRReplicationGraph Replication =====================
*
*	Overview
*	
*		This changes the way actor relevancy works. AActor::IsNetRelevantFor is NOT used in this system!
*		
*		Instead, The UFightingVRReplicationGraph contains UReplicationGraphNodes. These nodes are responsible for generating lists of actors to replicate for each connection.
...
*		
*		UFightingVRReplicationGraph is intended to not be directly used by the game code. That is, you should not have to include FightingVRReplicationGraph.h anywhere else.
*		Rather, UFightingVRReplicationGraph depends on the game code and registers for events that the game code broadcasts (e.g., events for players joining/leaving teams).
*		This choice was made because it gives UFightingVRReplicationGraph a complete holistic view of actor replication. Rather than exposing generic public functions that any
*		place in game code can invoke, all notifications are explicitly registered in UFightingVRReplicationGraph::InitGlobalActorClassSettings.
*		
*	FightingVR Nodes
*	
...
*		
*		UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection
...
*		owning connection only) via UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection.
*		
...
*		
*	Dependent Actors (AFightingVRWeapon)
*		
*		Replication Graph introduces a concept of dependent actor replication. This is an actor (AFightingVRWeapon) that only replicates when another actor replicates (Pawn). I.e, the weapon
...
*		
*		See UFightingVRReplicationGraph::OnCharacterWeaponChange: this is how actors are added/removed from the dependent actor list. 
*	
*	How To Use
*	
...
*		
*		Making something always relevant to connection: You will need to modify UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection. You will also want 
*		to make sure the actor does not get put in one of the other nodes. The safest way to do this is by setting its EClassRepNodeMapping to NotRouted in UFightingVRReplicationGraph::InitGlobalActorClassSettings.
*
*	How To Debug
*	
...
*		
*		FightingVRRepGraph.PrintRouting - will print the EClassRepNodeMapping for each class. That is, how a given actor class is routed (or not) in the Replication Graph.
*	
*/

#include "FightingVRReplicationGraph.h"
...
DEFINE_LOG_CATEGORY( LogFightingVRReplicationGraph );

float CVar_FightingVRRepGraph_DestructionInfoMaxDist = 30000.f;
static FAutoConsoleVariableRef CVarFightingVRRepGraphDestructMaxDist(TEXT("FightingVRRepGraph.DestructInfo.MaxDist"), CVar_FightingVRRepGraph_DestructionInfoMaxDist, TEXT("Max distance (not squared) to rep destruct infos at"), ECVF_Default );

int32 CVar_FightingVRRepGraph_DisplayClientLevelStreaming = 0;
static FAutoConsoleVariableRef CVarFightingVRRepGraphDisplayClientLevelStreaming(TEXT("FightingVRRepGraph.DisplayClientLevelStreaming"), CVar_FightingVRRepGraph_DisplayClientLevelStreaming, TEXT(""), ECVF_Default );

float CVar_FightingVRRepGraph_CellSize = 10000.f;
static FAutoConsoleVariableRef CVarFightingVRRepGraphCellSize(TEXT("FightingVRRepGraph.CellSize"), CVar_FightingVRRepGraph_CellSize, TEXT(""), ECVF_Default );
...
float CVar_FightingVRRepGraph_SpatialBiasX = -150000.f;
static FAutoConsoleVariableRef CVarFightingVRRepGraphSpatialBiasX(TEXT("FightingVRRepGraph.SpatialBiasX"), CVar_FightingVRRepGraph_SpatialBiasX, TEXT(""), ECVF_Default );
...
float CVar_FightingVRRepGraph_SpatialBiasY = -200000.f;
static FAutoConsoleVariableRef CVarFightingVRRepSpatialBiasY(TEXT("FightingVRRepGraph.SpatialBiasY"), CVar_FightingVRRepGraph_SpatialBiasY, TEXT(""), ECVF_Default );
...
int32 CVar_FightingVRRepGraph_DynamicActorFrequencyBuckets = 3;
static FAutoConsoleVariableRef CVarFightingVRRepDynamicActorFrequencyBuckets(TEXT("FightingVRRepGraph.DynamicActorFrequencyBuckets"), CVar_FightingVRRepGraph_DynamicActorFrequencyBuckets, TEXT(""), ECVF_Default );

int32 CVar_FightingVRRepGraph_DisableSpatialRebuilds = 1;
static FAutoConsoleVariableRef CVarFightingVRRepDisableSpatialRebuilds(TEXT("FightingVRRepGraph.DisableSpatialRebuilds"), CVar_FightingVRRepGraph_DisableSpatialRebuilds, TEXT(""), ECVF_Default );

// ----------------------------------------------------------------------------------------------------------


UFightingVRReplicationGraph::UFightingVRReplicationGraph()
{
}
...
void InitClassReplicationInfo(FClassReplicationInfo& Info, UClass* Class, bool bSpatialize, float ServerMaxTickRate)
{
	...
	if (bSpatialize)
	{
		...
		UE_LOG(LogFightingVRReplicationGraph, Log, TEXT("Setting cull distance for %s to %f (%f)"), *Class->GetName(), Info.GetCullDistanceSquared(), Info.GetCullDistance());
	}
	...
	UE_LOG(LogFightingVRReplicationGraph, Log, TEXT("Setting replication period for %s (%s) to %d frames (%.2f)"), *Class->GetName(), *NativeClass->GetName(), Info.ReplicationPeriodFrame, CDO->NetUpdateFrequency);
}

void UFightingVRReplicationGraph::ResetGameWorldState()
{
	...
	for (UNetReplicationGraphConnection* ConnManager : Connections)
	{
		...
		{
			if (UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = Cast<UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection>(ConnectionNode))
			{
...
void UFightingVRReplicationGraph::InitGlobalActorClassSettings()
{
	...
	AddInfo( AFightingVRWeapon::StaticClass(),							EClassRepNodeMapping::NotRouted);				// Handled via DependantActor replication (Pawn)
	AddInfo( ALevelScriptActor::StaticClass(),						EClassRepNodeMapping::NotRouted);				// Not needed
	AddInfo( APlayerState::StaticClass(),							EClassRepNodeMapping::NotRouted);				// Special cased via UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter
	...
	AddInfo( AFightingVRPickup::StaticClass(),							EClassRepNodeMapping::Spatialize_Static);		// Spatialized and never moves. Routes to GridNode.

#if WITH_GAMEPLAY_DEBUGGER
	AddInfo( AGameplayDebuggerCategoryReplicator::StaticClass(),	EClassRepNodeMapping::NotRouted);				// Replicated via UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection
#endif
	...
	for (TObjectIterator<UClass> It; It; ++It)
	{
		...
		if (AActor* SuperCDO = Cast<AActor>(SuperClass->GetDefaultObject()))
		{
			...
			{
				UE_LOG(LogFightingVRReplicationGraph, Log, TEXT("Adding %s to NonSpatializedChildClasses. (Parent: %s)"), *GetLegacyDebugStr(ActorCDO), *GetLegacyDebugStr(SuperCDO));
	...
	UE_LOG(LogFightingVRReplicationGraph, Log, TEXT(""));
	UE_LOG(LogFightingVRReplicationGraph, Log, TEXT("Class Routing Map: "));
	...
	{		
		...
		UE_LOG(LogFightingVRReplicationGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(ParentNativeClass), *Enum->GetNameStringByValue(static_cast<uint32>(Mapping)));
	}

	UE_LOG(LogFightingVRReplicationGraph, Log, TEXT(""));
	UE_LOG(LogFightingVRReplicationGraph, Log, TEXT("Class Settings Map: "));
	...
	for (auto ClassRepInfoIt = GlobalActorReplicationInfoMap.CreateClassMapIterator(); ClassRepInfoIt; ++ClassRepInfoIt)
	{
		...
		UE_LOG(LogFightingVRReplicationGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(GetParentNativeClass(Class)), *ClassInfo.BuildDebugStringDelta());
	}


	// Rep destruct infos based on CVar value
	DestructInfoMaxDistanceSquared = CVar_FightingVRRepGraph_DestructionInfoMaxDist * CVar_FightingVRRepGraph_DestructionInfoMaxDist;

	// -------------------------------------------------------
	//	Register for game code callbacks.
	//	This could have been done the other way: E.g, AMyGameActor could do GetNetDriver()->GetReplicationDriver<UFightingVRReplicationGraph>()->OnMyGameEvent etc.
	...
	AFightingVRCharacter::NotifyEquipWeapon.AddUObject(this, &UFightingVRReplicationGraph::OnCharacterEquipWeapon);
	AFightingVRCharacter::NotifyUnEquipWeapon.AddUObject(this, &UFightingVRReplicationGraph::OnCharacterUnEquipWeapon);

#if WITH_GAMEPLAY_DEBUGGER
	AGameplayDebuggerCategoryReplicator::NotifyDebuggerOwnerChange.AddUObject(this, &UFightingVRReplicationGraph::OnGameplayDebuggerOwnerChange);
#endif
}

void UFightingVRReplicationGraph::InitGlobalGraphNodes()
{
	...
	GridNode->CellSize = CVar_FightingVRRepGraph_CellSize;
	GridNode->SpatialBias = FVector2D(CVar_FightingVRRepGraph_SpatialBiasX, CVar_FightingVRRepGraph_SpatialBiasY);

	if (CVar_FightingVRRepGraph_DisableSpatialRebuilds)
	{
	...
	// -----------------------------------------------
	UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter* PlayerStateNode = CreateNewNode<UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter>();
	AddGlobalGraphNode(PlayerStateNode);
}

void UFightingVRReplicationGraph::InitConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection)
{
	Super::InitConnectionGraphNodes(RepGraphConnection);

	UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = CreateNewNode<UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection>();

	// This node needs to know when client levels go in and out of visibility
	RepGraphConnection->OnClientVisibleLevelNameAdd.AddUObject(AlwaysRelevantConnectionNode, &UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd);
	RepGraphConnection->OnClientVisibleLevelNameRemove.AddUObject(AlwaysRelevantConnectionNode, &UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove);

	AddConnectionGraphNode(AlwaysRelevantConnectionNode, RepGraphConnection);
}

EClassRepNodeMapping UFightingVRReplicationGraph::GetMappingPolicy(UClass* Class)
{
...
void UFightingVRReplicationGraph::RouteAddNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo)
{
...
void UFightingVRReplicationGraph::RouteRemoveNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo)
{
	EClassRepNodeMapping Policy = GetMappingPolicy(ActorInfo.Class);
	switch(Policy)
	{
		...
		case EClassRepNodeMapping::RelevantAllConnections:
		{
			...
			else
			{
				...
				{
					UE_LOG(LogFightingVRReplicationGraph, Warning, TEXT("Actor %s was not found in AlwaysRelevantStreamingLevelActors list. LevelName: %s"), *GetActorRepListTypeDebugString(ActorInfo.Actor), *ActorInfo.StreamingLevelName.ToString());
				}				
			}
			break;
		}
...
void UFightingVRReplicationGraph::OnCharacterEquipWeapon(AFightingVRCharacter* Character, AFightingVRWeapon* NewWeapon)
{
...
void UFightingVRReplicationGraph::OnCharacterUnEquipWeapon(AFightingVRCharacter* Character, AFightingVRWeapon* OldWeapon)
{
...
#if WITH_GAMEPLAY_DEBUGGER
void UFightingVRReplicationGraph::OnGameplayDebuggerOwnerChange(AGameplayDebuggerCategoryReplicator* Debugger, APlayerController* OldOwner)
{
	auto GetAlwaysRelevantForConnectionNode = [&](APlayerController* Controller) -> UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection*
	{
		if (OldOwner)
		{
	...
	if (UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(OldOwner))
	{
		AlwaysRelevantConnectionNode->GameplayDebugger = nullptr;
	}

	if (UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(Debugger->GetReplicationOwner()))
	{
...
void UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::ResetGameWorldState()
{
	AlwaysRelevantStreamingLevelsNeedingReplication.Empty();
}

void UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	QUICK_SCOPE_CYCLE_COUNTER( UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection_GatherActorListsForConnection );

	UFightingVRReplicationGraph* FightingVRGraph = CastChecked<UFightingVRReplicationGraph>(GetOuter());
	...
	auto ResetActorCullDistance = [&](AActor* ActorToSet, AActor*& LastActor) {
		...
		if (ActorToSet != LastActor)
		{
			...
			UE_LOG(LogFightingVRReplicationGraph, Verbose, TEXT("Setting pawn cull distance to 0. %s"), *ActorToSet->GetName());
	...
	for (const FNetViewer& CurViewer : Params.Viewers)
	{
		...
		if (AFightingVRPlayerController* PC = Cast<AFightingVRPlayerController>(CurViewer.InViewer))
		{
			...
			if (bReplicatePS)
			{
				// Always return the player state to the owning player. Simulated proxy player states are handled by UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter
			...
			if (AFightingVRCharacter* Pawn = Cast<AFightingVRCharacter>(PC->GetPawn()))
			{
				...
				{
					AFightingVRWeapon* Weapon = Pawn->GetInventoryWeapon(i);
					if (Weapon)
					{
			...
			if (AFightingVRCharacter* ViewTargetPawn = Cast<AFightingVRCharacter>(CurViewer.ViewTarget))
			{
	...
	TMap<FName, FActorRepListRefView>& AlwaysRelevantStreamingLevelActors = FightingVRGraph->AlwaysRelevantStreamingLevelActors;
	...
	for (int32 Idx=AlwaysRelevantStreamingLevelsNeedingReplication.Num()-1; Idx >= 0; --Idx)
	{
		...
		if (Ptr == nullptr)
		{
			...
			UE_CLOG(CVar_FightingVRRepGraph_DisplayClientLevelStreaming > 0, LogFightingVRReplicationGraph, Display, TEXT("CLIENTSTREAMING Removing %s from AlwaysRelevantStreamingLevelActors because FActorRepListRefView is null. %s "), *StreamingLevel.ToString(),  *Params.ConnectionManager.GetName());
		...
		if (RepList.Num() > 0)
		{
			...
			if (bAllDormant)
			{
				UE_CLOG(CVar_FightingVRRepGraph_DisplayClientLevelStreaming > 0, LogFightingVRReplicationGraph, Display, TEXT("CLIENTSTREAMING All AlwaysRelevant Actors Dormant on StreamingLevel %s for %s. Removing list."), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				AlwaysRelevantStreamingLevelsNeedingReplication.RemoveAtSwap(Idx, 1, false);
			}
			else
			{
				UE_CLOG(CVar_FightingVRRepGraph_DisplayClientLevelStreaming > 0, LogFightingVRReplicationGraph, Display, TEXT("CLIENTSTREAMING Adding always Actors on StreamingLevel %s for %s because it has at least one non dormant actor"), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				Params.OutGatheredReplicationLists.AddReplicationActorList(RepList);
			}
		}
		else
		{
			UE_LOG(LogFightingVRReplicationGraph, Warning, TEXT("UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection - empty RepList %s"), *Params.ConnectionManager.GetName());
		}
...
void UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd(FName LevelName, UWorld* StreamingWorld)
{
	UE_CLOG(CVar_FightingVRRepGraph_DisplayClientLevelStreaming > 0, LogFightingVRReplicationGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityAdd - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Add(LevelName);
}

void UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove(FName LevelName)
{
	UE_CLOG(CVar_FightingVRRepGraph_DisplayClientLevelStreaming > 0, LogFightingVRReplicationGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityRemove - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Remove(LevelName);
}

void UFightingVRReplicationGraphNode_AlwaysRelevant_ForConnection::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	...
	{
		UFightingVRReplicationGraph* FightingVRGraph = CastChecked<UFightingVRReplicationGraph>(GetOuter());
		if (FActorRepListRefView* RepList = FightingVRGraph->AlwaysRelevantStreamingLevelActors.Find(LevelName))
		{
...
UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter::UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter()
{
	bRequiresPrepareForReplicationCall = true;
}

void UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter::PrepareForReplication()
{
	QUICK_SCOPE_CYCLE_COUNTER( UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter_GlobalPrepareForReplication );
...
void UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
...
void UFightingVRReplicationGraphNode_PlayerStateFrequencyLimiter::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	DebugInfo.Log(NodeName);
	DebugInfo.PushIndent();	
...
void UFightingVRReplicationGraph::PrintRepNodePolicies()
{
	UEnum* Enum = StaticEnum<EClassRepNodeMapping>();
	if (!Enum)
	{
	...
	GLog->Logf(TEXT("===================================="));
	GLog->Logf(TEXT("FightingVR Replication Routing Policies"));
	GLog->Logf(TEXT("===================================="));
...
FAutoConsoleCommandWithWorldAndArgs FightingVRPrintRepNodePoliciesCmd(TEXT("FightingVRRepGraph.PrintRouting"),TEXT("Prints how actor classes are routed to RepGraph nodes"),
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
	{
		for (TObjectIterator<UFightingVRReplicationGraph> It; It; ++It)
		{
...
FAutoConsoleCommandWithWorldAndArgs ChangeFrequencyBucketsCmd(TEXT("FightingVRRepGraph.FrequencyBuckets"), TEXT("Resets frequency bucket count."), FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray< FString >& Args, UWorld* World) 
{
	int32 Buckets = 1;
	if (Args.Num() > 0)
	{
	...
	UE_LOG(LogFightingVRReplicationGraph, Display, TEXT("Setting Frequency Buckets to %d"), Buckets);
