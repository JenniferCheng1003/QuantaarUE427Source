...
AFightingVRSession::AFightingVRSession(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &AFightingVRSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AFightingVRSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &AFightingVRSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &AFightingVRSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &AFightingVRSession::OnStartOnlineGameComplete);
	}
}
 ...
 */
void AFightingVRSession::OnStartOnlineGameComplete(FName InSessionName, bool bWasSuccessful)
{
...
void AFightingVRSession::HandleMatchHasStarted()
{
 ...
 *
 */
void AFightingVRSession::HandleMatchHasEnded()
{
...
bool AFightingVRSession::IsBusy() const
{ 
...
EOnlineAsyncTaskState::Type AFightingVRSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
 ...
 */
const TArray<FOnlineSessionSearchResult> & AFightingVRSession::GetSearchResults() const
{ 
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AFightingVRSession::OnCreateSessionComplete(FName InSessionName, bool bWasSuccessful)
{
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AFightingVRSession::OnDestroySessionComplete(FName InSessionName, bool bWasSuccessful)
{
...
bool AFightingVRSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
	...
	if (OnlineSub)
	{
		...
		{
			HostSettings = MakeShareable(new FFightingVROnlineSessionSettings(bIsLAN, bIsPresence, MaxPlayers));
...
bool AFightingVRSession::HostSession(const TSharedPtr<const FUniqueNetId> UserId, const FName InSessionName, const FOnlineSessionSettings& SessionSettings)
{
	bool bResult = false;
...
void AFightingVRSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void AFightingVRSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void AFightingVRSession::ChooseBestSession()
{
...
void AFightingVRSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void AFightingVRSession::ContinueMatchmaking()
{	
	ChooseBestSession();
...
void AFightingVRSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void AFightingVRSession::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = Online::GetSubsystem(GetWorld());
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && CurrentSessionParams.UserId.IsValid())
		{
			SearchSettings = MakeShareable(new FFightingVROnlineSearchSettings(bIsLAN, bIsPresence));
...
bool AFightingVRSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool AFightingVRSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FOnlineSessionSearchResult& SearchResult)
{
	bool bResult = false;
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AFightingVRSession::OnJoinSessionComplete(FName InSessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool AFightingVRSession::TravelToSession(int32 ControllerId, FName InSessionName)
{
	IOnlineSubsystem* OnlineSub = Online::GetSubsystem(GetWorld());
	if (OnlineSub)
	{
...
void AFightingVRSession::RegisterServer()
{
	IOnlineSubsystem* OnlineSub = Online::GetSubsystem(GetWorld());
	if (OnlineSub)
	{
		...
		{
			TSharedPtr<class FFightingVROnlineSessionSettings> FightingVRHostSettings = MakeShareable(new FFightingVROnlineSessionSettings(false, false, 16));
			FightingVRHostSettings->Set(SETTING_MATCHING_HOPPER, FString("TeamDeathmatch"), EOnlineDataAdvertisementType::DontAdvertise);
			FightingVRHostSettings->Set(SETTING_MATCHING_TIMEOUT, 120.0f, EOnlineDataAdvertisementType::ViaOnlineService);
			FightingVRHostSettings->Set(SETTING_SESSION_TEMPLATE_NAME, FString("GameSession"), EOnlineDataAdvertisementType::DontAdvertise);
			FightingVRHostSettings->Set(SETTING_GAMEMODE, FString("TeamDeathmatch"), EOnlineDataAdvertisementType::ViaOnlineService);
			FightingVRHostSettings->Set(SETTING_MAPNAME, GetWorld()->GetMapName(), EOnlineDataAdvertisementType::ViaOnlineService);
			FightingVRHostSettings->bAllowInvites = true;
			FightingVRHostSettings->bIsDedicated = true;
			if (FParse::Param(FCommandLine::Get(), TEXT("forcelan")))
			{
				UE_LOG(LogOnlineGame, Log, TEXT("Registering server as a LAN server"));
				FightingVRHostSettings->bIsLANMatch = true;
			}
			HostSettings = FightingVRHostSettings;
