...
FText FightingVRUIHelpers::GetProfileOpenText() const
{
...
bool FightingVRUIHelpers::ProfileOpenedUI(UWorld* World, const FUniqueNetId& Requestor, const FUniqueNetId& Requestee, const FOnProfileUIClosedDelegate* Delegate) const
{
...
FText FightingVRUIHelpers::GetProfileSwapText() const
{
	// @todo: replace button with icon
#if FIGHTINGVR_XBOX_STRINGS
...
bool FightingVRUIHelpers::ProfileSwapUI(UWorld* World, const int ControllerIndex, bool bShowOnlineOnly, const FOnLoginUIClosedDelegate* Delegate) const
{
	const IOnlineSubsystem* OnlineSub = Online::GetSubsystem(World);
	if (OnlineSub)
	{
