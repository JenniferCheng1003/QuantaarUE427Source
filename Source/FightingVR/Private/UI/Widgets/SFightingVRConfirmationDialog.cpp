...
#if !defined(FightingVR_CONTROLLER_DISCONNECT_STRICT)
	#define FightingVR_CONTROLLER_DISCONNECT_STRICT 0
#endif

void SFightingVRConfirmationDialog::Construct( const FArguments& InArgs )
{	
	...
	[					
		...
		[
			...
			[
				SNew( SButton )
				.ContentPadding(100)
				.OnClicked(this, &SFightingVRConfirmationDialog::OnConfirmHandler)
...
bool SFightingVRConfirmationDialog::SupportsKeyboardFocus() const
{
	return true;
}

FReply SFightingVRConfirmationDialog::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
	return FReply::Handled().ReleaseMouseCapture().SetUserFocus(SharedThis(this), EFocusCause::SetDirectly, true);
}

FReply SFightingVRConfirmationDialog::OnConfirmHandler()
{
	return ExecuteConfirm(FSlateApplication::Get().GetUserIndexForKeyboard());
}

FReply SFightingVRConfirmationDialog::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& KeyEvent)
{
	...
	switch (DialogType)
	{
		case EFightingVRDialogType::Generic:
		{
		...
		case EFightingVRDialogType::ControllerDisconnected:
		{
...
FReply SFightingVRConfirmationDialog::ExecuteConfirm(const int32 UserIndex)
{
	if (OnConfirm.IsBound())
	{
		//these two cases should be combined when we move to using PlatformUserIDs rather than ControllerIDs.
#if PLATFORM_PS4 || FightingVR_CONTROLLER_DISCONNECT_STRICT
		bool bExecute = false;
		// For controller reconnection, bind the confirming controller to the owner of this dialog
		if (DialogType == EFightingVRDialogType::ControllerDisconnected && PlayerOwner != nullptr)
		{
...
#if FightingVR_CONTROLLER_PAIRING_PROMPT_FOR_NEW_USER_WHEN_RECONNECTING
