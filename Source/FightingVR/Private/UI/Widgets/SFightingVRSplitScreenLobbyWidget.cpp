...
#include "SFightingVRSplitScreenLobbyWidget.h"
...
#define LOCTEXT_NAMESPACE "FightingVR.SplitScreenLobby"

int32 GFightingVRSplitScreenMax = 2;
static FAutoConsoleVariableRef CVarFightingVRSplitScreenMax(
	TEXT("r.FightingVRSplitScreenMax"),
	GFightingVRSplitScreenMax,
...
void SFightingVRSplitScreenLobby::Construct( const FArguments& InArgs )
{
#if PLATFORM_PS4
...
#if PLATFORM_SWITCH
	PressToPlayText = LOCTEXT("PressToPlay", "<img src=\"FightingVR.Switch.Right\"/> Select User");
	PressToFindText = LOCTEXT("PressToFind", "Press <img src=\"FightingVR.Switch.Right\"/> to Find Match");
	PressToStartMatchText = LOCTEXT("PressToStart", "<img src=\"FightingVR.Switch.Right\"/> Start Match / <img src=\"FightingVR.Switch.Up\"/> Connect Controllers");
	PlayAsGuestText = LOCTEXT("PlayAsGuest", "<img src=\"FightingVR.Switch.Left\"/> Play As Guest");
#endif	
	...
	const FFightingVRMenuStyle* ItemStyle = &FFightingVRStyle::Get().GetWidgetStyle<FFightingVRMenuStyle>("DefaultFightingVRMenuStyle");
...
//								.ColorAndOpacity(MenuTitleTextColor)
							...
							[							
								SNew(SRichTextBlock)
								.TextStyle(FFightingVRStyle::Get(), "FightingVR.SplitScreenLobby.StartMatchTextStyle")							
								.Text(this, &SFightingVRSplitScreenLobby::GetPlayFindText)
								.DecoratorStyleSet(&FFightingVRStyle::Get())
								+ SRichTextBlock::ImageDecorator()
							]
						]					
					...
					[
						...
						[
							...
							[							
								SNew(SRichTextBlock)
								.TextStyle(FFightingVRStyle::Get(), "FightingVR.SplitScreenLobby.StartMatchTextStyle")							
								.Text(this, &SFightingVRSplitScreenLobby::GetPlayAsGuestText)
								.DecoratorStyleSet(&FFightingVRStyle::Get())
								+ SRichTextBlock::ImageDecorator()
							]
						]	
...
void SFightingVRSplitScreenLobby::Clear()
{	
...
void SFightingVRSplitScreenLobby::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	// Parent tick
...
void SFightingVRSplitScreenLobby::UpdateSlots()
{
	if (GetGameInstance() == nullptr)
	{
		return;
...
void SFightingVRSplitScreenLobby::ConditionallyReadyPlayer( const int ControllerId, const bool bCanShowUI )
{
	UFightingVRInstance* const GameInstance = GetGameInstance();
		...
		( bFoundExistingPlayer || !bIsPlayerOnline ) )
	{
		...
		{
			ExternalUI->ShowLoginUI( ControllerId, OnlineMode == EOnlineMode::Online, false, FOnLoginUIClosedDelegate::CreateSP( this, &SFightingVRSplitScreenLobby::HandleLoginUIClosedAndReady ) );
		}
			...
			if(Identity.IsValid())
			{
				...
				Identity->GetUserPrivilege(
					*UniqueNetId,
					...
					IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate::CreateSP(this, &SFightingVRSplitScreenLobby::OnUserCanPlay));
			}
		}
	}
	else
...
void SFightingVRSplitScreenLobby::ReadyPlayer( const int ControllerId )
{
	FString Error;
...
void SFightingVRSplitScreenLobby::UnreadyPlayer( const int ControllerId )
{
	if (GetGameInstance() == nullptr)
	{
		return;
	...
	if ( LocalPlayer == NULL )
	{
		UE_LOG( LogOnline, Warning, TEXT( "SFightingVRSplitScreenLobby::UnreadyPlayer: ControllerId not found: %i" ), ControllerId );
		return;
	}
...
FReply SFightingVRSplitScreenLobby::OnOkOrCancel()
{
	UFightingVRViewportClient* FightingVRViewport = Cast<UFightingVRViewportClient>(GetGameInstance()->GetGameViewportClient());
...
bool SFightingVRSplitScreenLobby::ConfirmSponsorsSatisfied() const
{
	if (GetGameInstance() == nullptr)
	{
		return false;
...
void SFightingVRSplitScreenLobby::OnUserCanPlay(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, uint32 PrivilegeResults)
{
			...
			if (Identity.IsValid())
			{
				...
				if (FightingVRViewport != NULL)
				{
					FightingVRViewport->ShowDialog(
						...
						FText::GetEmpty(),
						FOnClicked::CreateRaw(this, &SFightingVRSplitScreenLobby::OnOkOrCancel),
						FOnClicked::CreateRaw(this, &SFightingVRSplitScreenLobby::OnOkOrCancel)
						);
				}
			}
...
int32 SFightingVRSplitScreenLobby::GetNumSupportedSlots() const
{
	return FMath::Clamp( GFightingVRSplitScreenMax, 1, MAX_POSSIBLE_SLOTS );
}

bool SFightingVRSplitScreenLobby::IsUniqueIdOnline( const FUniqueNetId& UniqueId ) const
{
	if (GetGameInstance() == nullptr)
	{
		return false;
...
FReply SFightingVRSplitScreenLobby::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	const UFightingVRInstance* GameInstance = GetGameInstance();
	if (GameInstance == nullptr)
	{
...
FReply SFightingVRSplitScreenLobby::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
...
void SFightingVRSplitScreenLobby::OnFocusLost( const FFocusEvent& InFocusEvent )
{
}

void SFightingVRSplitScreenLobby::HandleLoginUIClosedAndReady( TSharedPtr<const FUniqueNetId> UniqueId, const int UserIndex, const FOnlineError& Error )
{
	const UFightingVRInstance* GameInstance = GetGameInstance();
	if (GameInstance == nullptr)
	{
	...
	if ( UniqueId.IsValid() )
	{
		...
		else if (!IsUniqueIdOnline(*UniqueId))
		{
			if (OnlineMode == EOnlineMode::Online)
			{
				...
				OnLoginCompleteDelegateHandle = Online::GetIdentityInterface(World)->AddOnLoginCompleteDelegate_Handle(UserIndex, FOnLoginCompleteDelegate::CreateRaw(this, &SFightingVRSplitScreenLobby::OnLoginComplete));
...
void SFightingVRSplitScreenLobby::OnLoginComplete(int32 LocalUserNum, bool bWasSuccessful, const FUniqueNetId& UserId, const FString& Error)
{
...
UFightingVRInstance * SFightingVRSplitScreenLobby::GetGameInstance() const
{
	...
	check( PlayerOwner->GetGameInstance() == nullptr || CastChecked< UFightingVRInstance >( PlayerOwner->GetGameInstance() ) != nullptr );

	return Cast< UFightingVRInstance >( PlayerOwner->GetGameInstance() );
}

FText SFightingVRSplitScreenLobby::GetPlayFindText() const
{
	return bIsJoining ? PressToFindText : PressToStartMatchText;
}

#if PLATFORM_SWITCH
FText SFightingVRSplitScreenLobby::GetPlayAsGuestText() const
{
	const UFightingVRInstance* GameInstance = GetGameInstance();
	if (GameInstance == nullptr)
	{
