...
#include "SFightingVROnlineStore.h"
...
void SFightingVROnlineStore::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			...
			[
				...
				.OnGenerateRow(this, &SFightingVROnlineStore::MakeListViewWidget)
				.OnSelectionChanged(this, &SFightingVROnlineStore::EntrySelectionChanged)
				.OnMouseButtonDoubleClick(this,&SFightingVROnlineStore::OnListItemDoubleClicked)
				.HeaderRow(
					SNew(SHeaderRow)
		...
		[
			...
			[
				SNew(SRichTextBlock)
				.Text(this, &SFightingVROnlineStore::GetBottomText)
...
FText SFightingVROnlineStore::GetBottomText() const
{
	return StatusText;
}
 ...
 */
void SFightingVROnlineStore::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	SCompoundWidget::Tick( AllottedGeometry, InCurrentTime, InDeltaTime );
}

/** Returns logged in user */
TSharedPtr<const FUniqueNetId> SFightingVROnlineStore::GetLoggedInUser()
{
...
void SFightingVROnlineStore::BeginGettingOffers()
{
				...
				[this, StoreV2Int, PurchaseInt, UserId](bool bWasSuccessful, const TArray<FUniqueOfferId>& OfferIds, const FString& ErrorString)
			{
				...
				if (bWasSuccessful)
				{
					...
					for (const FOnlineStoreOfferRef& OfferRef : StoreOffers)
					{
						...
						NewOffer->Title = OfferRef->Title.IsEmptyOrWhitespace() ? NSLOCTEXT("FightingVROnlineStore", "DefaultOfferTitle", "EmptyTitle") : OfferRef->Title;
						NewOffer->Description = OfferRef->Description.IsEmptyOrWhitespace() ? NSLOCTEXT("FightingVROnlineStore", "DefaultOfferDescription", "EmptyDescription") : OfferRef->Description;
						NewOffer->Price = OfferRef->GetDisplayPrice().IsEmptyOrWhitespace() ? NSLOCTEXT("FightingVROnlineStore", "DefaultOfferDescription", "EmptyPrice") : OfferRef->GetDisplayPrice();
...
void SFightingVROnlineStore::SetStoreState(EStoreState NewState)
{
	...
	switch (State)
	{
		case EStoreState::PurchasingAnOffer:
			StatusText = FText(NSLOCTEXT("FightingVROnlineStore", "Status", "Purchasing..."));
			break;

		case EStoreState::GettingOffers:
			StatusText = FText(NSLOCTEXT("FightingVROnlineStore", "Status", "Checking what's available..."));
			break;

		case EStoreState::Browsing:
			if (OfferList.Num() == 0)
			{
				StatusText = FText(NSLOCTEXT("FightingVROnlineStore", "Status", "No offers found - press Space to refresh"));
				break;
			}
...
void SFightingVROnlineStore::PurchaseOffer()
{
	if (State != EStoreState::Browsing)
	{
...
void SFightingVROnlineStore::MarkAsPurchased(const TArray<FUniqueOfferId> & Offers)
{
...
void SFightingVROnlineStore::OnFocusLost( const FFocusEvent& InFocusEvent )
{
...
FReply SFightingVROnlineStore::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
...
void SFightingVROnlineStore::EntrySelectionChanged(TSharedPtr<FStoreEntry> InItem, ESelectInfo::Type SelectInfo)
{
	SelectedItem = InItem;
}

void SFightingVROnlineStore::OnListItemDoubleClicked(TSharedPtr<FStoreEntry> InItem)
{
	SelectedItem = InItem;
	PurchaseOffer();
	FSlateApplication::Get().SetKeyboardFocus(SharedThis(this));
...
void SFightingVROnlineStore::MoveSelection(int32 MoveBy)
{
...
FReply SFightingVROnlineStore::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent) 
{
...
TSharedRef<ITableRow> SFightingVROnlineStore::MakeListViewWidget(TSharedPtr<FStoreEntry> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	public:
		...
		TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName)
		{
			...
			return SNew(STextBlock)
				.Text(ItemText)
				.TextStyle(FFightingVRStyle::Get(), "FightingVR.MenuStoreListTextStyle");
		}
