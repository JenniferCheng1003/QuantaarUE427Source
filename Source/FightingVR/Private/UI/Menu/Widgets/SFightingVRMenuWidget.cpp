...
#include "SFightingVRMenuWidget.h"
#include "FightingVR.h"
#include "Engine/Console.h"
#include "FightingVRMenuItem.h"
...
#define LOCTEXT_NAMESPACE "SFightingVRMenuWidget"

#if !defined(PROFILE_SWAPPING)
#define PROFILE_SWAPPING	0
#endif

void SFightingVRMenuWidget::Construct(const FArguments& InArgs)
{
	MenuStyle = &FFightingVRStyle::Get().GetWidgetStyle<FFightingVRMenuStyle>("DefaultFightingVRMenuStyle");
	...
	Visibility.Bind(this, &SFightingVRMenuWidget::GetSlateVisibility);
	...
	const FText ProfileSwap = FightingVRUIHelpers::Get().GetProfileSwapText();
	const TSharedRef< FSlateFontMeasure > FontMeasure = FSlateApplication::Get().GetRenderer()->GetFontMeasureService();
	const FSlateFontInfo PlayerNameFontInfo = FFightingVRStyle::Get().GetWidgetStyle<FTextBlockStyle>("FightingVR.MenuProfileNameStyle").Font;
	const FSlateFontInfo ProfileSwapFontInfo = FFightingVRStyle::Get().GetWidgetStyle<FTextBlockStyle>("FightingVR.MenuServerListTextStyle").Font;
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(SImage)
					.Visibility(this, &SFightingVRMenuWidget::GetProfileSwapVisibility)
					.ColorAndOpacity(this, &SFightingVRMenuWidget::GetHeaderColor)
			...
			[
				...
				[
					SNew(STextBlock)
					.TextStyle(FFightingVRStyle::Get(), "FightingVR.MenuProfileNameStyle")
		...
		[
			...
			.HAlign(HAlign_Left)
			.VAlign(VAlign_Top)
			.Padding(TAttribute<FMargin>(this,&SFightingVRMenuWidget::GetMenuOffset))
			[
				SNew(SVerticalBox)
				+ SVerticalBox::Slot()
				.AutoHeight()
				[
					...
					[
						...
						[
							...
							.ColorAndOpacity(MenuTitleTextColor)
							.Text(this,&SFightingVRMenuWidget::GetMenuTitle)
						]
					]
				]
				+ SVerticalBox::Slot()
				...
				[
					...
					.ColorAndOpacity(this, &SFightingVRMenuWidget::GetBottomColor)
					.VAlign(VAlign_Top)
					.HAlign(HAlign_Left)
					[
						SNew(SHorizontalBox)
						...
						[
							...
							+ SVerticalBox::Slot()
							.AutoHeight()
							.Padding(TAttribute<FMargin>(this,&SFightingVRMenuWidget::GetLeftMenuOffset))
							[
								SNew(SBorder)
								.BorderImage(&MenuStyle->LeftBackgroundBrush)
								.BorderBackgroundColor(FLinearColor(1.0f, 1.0f, 1.0f, 1.0f))
								.Padding(FMargin(OutlineWidth))
								.DesiredSizeScale(this, &SFightingVRMenuWidget::GetBottomScale)
								.VAlign(VAlign_Top)
								.HAlign(HAlign_Left)
								[
						...
						[
							...
							+ SVerticalBox::Slot()
							.Padding(TAttribute<FMargin>(this,&SFightingVRMenuWidget::GetSubMenuOffset))
							.AutoHeight()
							[
								SNew(SBorder)
...
EVisibility SFightingVRMenuWidget::GetSlateVisibility() const
{
	return bConsoleVisible ? EVisibility::HitTestInvisible : EVisibility::Visible;
}

FText SFightingVRMenuWidget::GetMenuTitle() const 
{
	return CurrentMenuTitle;
}

FMargin SFightingVRMenuWidget::GetProfileSwapOffset() const
{
	return FMargin(0.0f, 50.0f, 50.0f, 0.0f);
}

bool SFightingVRMenuWidget::IsProfileSwapActive() const
{
#if PROFILE_SWAPPING
...
EVisibility SFightingVRMenuWidget::GetProfileSwapVisibility() const
{
	return IsProfileSwapActive() ? EVisibility::Visible : EVisibility::Collapsed;
}

UWorld* SFightingVRMenuWidget::GetWorld() const
{
	return PlayerOwner.IsValid() ? PlayerOwner->GetWorld() : nullptr;
}

bool SFightingVRMenuWidget::ProfileUISwap(const int ControllerIndex) const
{
	if (IsProfileSwapActive())
	{
		const FOnLoginUIClosedDelegate Delegate = FOnLoginUIClosedDelegate::CreateSP( const_cast<SFightingVRMenuWidget*>(this), &SFightingVRMenuWidget::HandleProfileUISwapClosed );
		if ( FightingVRUIHelpers::Get().ProfileSwapUI(GetWorld(), ControllerIndex, false, &Delegate) )
		{
			UFightingVRInstance* GameInstance = PlayerOwner.IsValid() ? Cast< UFightingVRInstance >( PlayerOwner->GetGameInstance() ) : nullptr;
...
void SFightingVRMenuWidget::HandleProfileUISwapClosed(TSharedPtr<const FUniqueNetId> UniqueId, const int ControllerIndex, const FOnlineError& Error)
{
	UFightingVRInstance * GameInstance = PlayerOwner.IsValid() ? Cast< UFightingVRInstance >( PlayerOwner->GetGameInstance() ) : nullptr;
	...
	UFightingVRLocalPlayer* LocalPlayer = Cast<UFightingVRLocalPlayer>(PlayerOwner.Get());
	LocalPlayer->LoadPersistentUser();
}

void SFightingVRMenuWidget::LockControls(bool bEnable)
{
	bControlsLocked = bEnable;
}

int32 SFightingVRMenuWidget::GetOwnerUserIndex()
{
	return PlayerOwner.IsValid() ? PlayerOwner->GetControllerId() : 0;
}

int32 SFightingVRMenuWidget::GetMenuLevel()
{
	return MenuHistory.Num();
}

void SFightingVRMenuWidget::BuildAndShowMenu()
{
...
void SFightingVRMenuWidget::HideMenu()
{
	if (!bMenuHiding)
	{
...
void SFightingVRMenuWidget::SetupAnimations()
{
	//Setup a curve
...
void SFightingVRMenuWidget::BuildLeftPanel(bool bInGoingBack)
{
	...
	if (bLeftMenuChanging)
	{
		...
		{
			FFightingVRMenuInfo MenuInfo = MenuHistory.Pop();
		...
		{
			MenuHistory.Push(FFightingVRMenuInfo(CurrentMenu, SelectedIndex, CurrentMenuTitle));
	...
	for(int32 i = 0; i < CurrentMenu.Num(); ++i)
	{
		if (CurrentMenu[i]->bVisible)
		{
			...
			if (CurrentMenu[i]->MenuItemType == EFightingVRMenuItemType::Standard)
			{
				TmpWidget = SAssignNew(CurrentMenu[i]->Widget, SFightingVRMenuItem)
					.PlayerOwner(PlayerOwner)
					.OnClicked(this, &SFightingVRMenuWidget::ButtonClicked, i)
					.Text(CurrentMenu[i]->GetText())
					.bIsMultichoice(false);
			} 
			else if (CurrentMenu[i]->MenuItemType == EFightingVRMenuItemType::MultiChoice)
			{
				TmpWidget = SAssignNew(CurrentMenu[i]->Widget, SFightingVRMenuItem)
					.PlayerOwner(PlayerOwner)
					.OnClicked(this, &SFightingVRMenuWidget::ButtonClicked, i)
					.Text(CurrentMenu[i]->GetText() )
					.bIsMultichoice(true)
					.OnArrowPressed(this, &SFightingVRMenuWidget::ChangeOption)
					.OptionText(this, &SFightingVRMenuWidget::GetOptionText, CurrentMenu[i]);
				UpdateArrows(CurrentMenu[i]);
			}
			else if (CurrentMenu[i]->MenuItemType == EFightingVRMenuItemType::CustomWidget)
			{
	...
	TSharedPtr<FFightingVRMenuItem> FirstMenuItem = CurrentMenu.IsValidIndex(SelectedIndex) ? CurrentMenu[SelectedIndex] : NULL;
	if (FirstMenuItem.IsValid() && FirstMenuItem->MenuItemType != EFightingVRMenuItemType::CustomWidget)
	{
...
FText SFightingVRMenuWidget::GetOptionText(TSharedPtr<FFightingVRMenuItem> MenuItem) const
{
...
void SFightingVRMenuWidget::BuildRightPanel()
{
	...
	for(int32 i = 0; i < NextMenu.Num(); ++i)
	{
		...
		{
			TSharedPtr<SFightingVRMenuItem> TmpButton;
			//Custom menu items are not supported in the right panel
			if (NextMenu[i]->MenuItemType == EFightingVRMenuItemType::Standard)
			{
				TmpButton = SAssignNew(NextMenu[i]->Widget, SFightingVRMenuItem)
					.PlayerOwner(PlayerOwner)
					.Text(NextMenu[i]->GetText())
					.InactiveTextAlpha(0.3f)
					.bIsMultichoice(false);
			}
			else if (NextMenu[i]->MenuItemType == EFightingVRMenuItemType::MultiChoice)
			{
				TmpButton = SAssignNew(NextMenu[i]->Widget, SFightingVRMenuItem)
					.PlayerOwner(PlayerOwner)
					.Text(NextMenu[i]->GetText() )
					.InactiveTextAlpha(0.3f)
					.bIsMultichoice(true)
					.OptionText(this, &SFightingVRMenuWidget::GetOptionText, NextMenu[i]);
			}
...
void SFightingVRMenuWidget::UpdateArrows(TSharedPtr<FFightingVRMenuItem> MenuItem)
{
...
void SFightingVRMenuWidget::EnterSubMenu()
{
...
void SFightingVRMenuWidget::MenuGoBack(bool bSilent)
{
...
void SFightingVRMenuWidget::ConfirmMenuItem()
{
...
void SFightingVRMenuWidget::ControllerFacebuttonLeftPressed()
{
...
void SFightingVRMenuWidget::ControllerUpInputPressed()
{
...
void SFightingVRMenuWidget::ControllerDownInputPressed()
{
...
void SFightingVRMenuWidget::ControllerFacebuttonDownPressed()
{
...
void SFightingVRMenuWidget::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	if (MenuWidgetAnimation.IsAtEnd())
	{
		if (bLeftMenuChanging)
		{
			...
			if (!LeftMenuWidgetAnimation.IsPlaying())
			{
				...
				if (CurrentMenu.Num() == 1 && CurrentMenu.Top()->MenuItemType == EFightingVRMenuItemType::CustomWidget)
				{
...
FMargin SFightingVRMenuWidget::GetMenuOffset() const
{
...
FMargin SFightingVRMenuWidget::GetLeftMenuOffset() const
{
...
FMargin SFightingVRMenuWidget::GetSubMenuOffset() const
{
...
FReply SFightingVRMenuWidget::ButtonClicked(int32 ButtonIndex)
{
	...
	{
		TSharedPtr<SFightingVRMenuItem> MenuItem = CurrentMenu[SelectedIndex]->Widget;
...
void SFightingVRMenuWidget::FadeIn()
{
...
FReply SFightingVRMenuWidget::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SFightingVRMenuWidget::ChangeOption(int32 MoveBy)
{
	TSharedPtr<FFightingVRMenuItem> MenuItem = CurrentMenu[SelectedIndex];

	const int32 MinIndex = MenuItem->MinMultiChoiceIndex > -1 ? MenuItem->MinMultiChoiceIndex : 0;
	const int32 MaxIndex = MenuItem->MaxMultiChoiceIndex > -1 ? MenuItem->MaxMultiChoiceIndex : MenuItem->MultiChoice.Num()-1;
	const int32 CurIndex = MenuItem->SelectedMultiChoice;

	if (MenuItem->MenuItemType == EFightingVRMenuItemType::MultiChoice)
	{
...
int32 SFightingVRMenuWidget::GetNextValidIndex(int32 MoveBy)
{
...
FReply SFightingVRMenuWidget::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
FReply SFightingVRMenuWidget::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
	//Focus the custom widget
	if (CurrentMenu.Num() == 1 && CurrentMenu.Top()->MenuItemType == EFightingVRMenuItemType::CustomWidget)
	{
