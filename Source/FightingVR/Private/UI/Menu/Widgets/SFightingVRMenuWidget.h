...
//class declare
class SFightingVRMenuWidget : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SFightingVRMenuWidget)
	: _PlayerOwner()
	, _IsGameMenu(false)
	{
	}
	...
	void UpdateArrows(TSharedPtr<FFightingVRMenuItem> MenuItem);
	...
	TArray<FFightingVRMenuInfo> MenuHistory;
...
private:
	...
	FText GetOptionText(TSharedPtr<FFightingVRMenuItem> MenuItem) const;
	...
	const struct FFightingVRMenuStyle *MenuStyle;
};

namespace MenuHelper
{
	FORCEINLINE void EnsureValid(TSharedPtr<FFightingVRMenuItem>& MenuItem)
	{
		if (!MenuItem.IsValid())
		{
			MenuItem = FFightingVRMenuItem::CreateRoot();
		}
	}

	//Helper functions for creating menu items
	FORCEINLINE TSharedRef<FFightingVRMenuItem> AddMenuItem(TSharedPtr<FFightingVRMenuItem>& MenuItem, const FText& Text)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFightingVRMenuItem> Item = MakeShareable(new FFightingVRMenuItem(Text));
	...
	FORCEINLINE TSharedRef<FFightingVRMenuItem> AddMenuItem(TSharedPtr<FFightingVRMenuItem>& MenuItem, const FText& Text, UserClass* inObj, typename FFightingVRMenuItem::FOnConfirmMenuItem::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFightingVRMenuItem> Item = MakeShareable(new FFightingVRMenuItem(Text));
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FFightingVRMenuItem> AddMenuItemSP(TSharedPtr<FFightingVRMenuItem>& MenuItem, const FText& Text, UserClass* inObj, typename FFightingVRMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFightingVRMenuItem> Item = MakeShareable(new FFightingVRMenuItem(Text));
	...
	FORCEINLINE TSharedRef<FFightingVRMenuItem> AddMenuOption(TSharedPtr<FFightingVRMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFightingVRMenuItem> Item = MakeShareable(new FFightingVRMenuItem(Text, OptionsList));
		MenuItem->SubMenu.Add(Item);
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FFightingVRMenuItem> AddMenuOption(TSharedPtr<FFightingVRMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FFightingVRMenuItem::FOnOptionChanged::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFightingVRMenuItem> Item = MakeShareable(new FFightingVRMenuItem(Text, OptionsList));
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FFightingVRMenuItem> AddMenuOptionSP(TSharedPtr<FFightingVRMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FFightingVRMenuItem::FOnOptionChanged::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FFightingVRMenuItem> Item = MakeShareable(new FFightingVRMenuItem(Text, OptionsList));
	...
	FORCEINLINE TSharedRef<FFightingVRMenuItem> AddExistingMenuItem(TSharedPtr<FFightingVRMenuItem>& MenuItem, TSharedRef<FFightingVRMenuItem> SubMenuItem)
	{
		EnsureValid(MenuItem);
	...
	FORCEINLINE TSharedRef<FFightingVRMenuItem> AddCustomMenuItem(TSharedPtr<FFightingVRMenuItem>& MenuItem, TSharedPtr<SWidget> CustomWidget)
	{
		EnsureValid(MenuItem);
		MenuItem->SubMenu.Add(MakeShareable(new FFightingVRMenuItem(CustomWidget)));
		return MenuItem->SubMenu.Last().ToSharedRef();
	}

	FORCEINLINE void ClearSubMenu(TSharedPtr<FFightingVRMenuItem>& MenuItem)
	{
		EnsureValid(MenuItem);
	...
	template< class UserClass >	
	FORCEINLINE void PlaySoundAndCall(UWorld* World, const FSlateSound& Sound, int32 UserIndex, UserClass* inObj, typename FFightingVRMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
