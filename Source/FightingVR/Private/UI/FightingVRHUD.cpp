...
const float AFightingVRHUD::MinHudScale = 0.5f;

AFightingVRHUD::AFightingVRHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	/*OnPlayerTalkingStateChangedDelegate = FOnPlayerTalkingStateChangedDelegate::CreateUObject(this, &AFightingVRHUD::OnPlayerTalkingStateChanged);
	...
	HitNotifyIcon[EFightingVRHudPosition::Left] = UCanvas::MakeIcon(HitNotifyTexture,  158, 831, 585, 392);	
	HitNotifyIcon[EFightingVRHudPosition::FrontLeft] = UCanvas::MakeIcon(HitNotifyTexture, 369, 434, 460, 378);	
	HitNotifyIcon[EFightingVRHudPosition::Front] = UCanvas::MakeIcon(HitNotifyTexture,  848, 284, 361, 395);	
	HitNotifyIcon[EFightingVRHudPosition::FrontRight] = UCanvas::MakeIcon(HitNotifyTexture,  1212, 397, 427, 394);	
	HitNotifyIcon[EFightingVRHudPosition::Right] = UCanvas::MakeIcon(HitNotifyTexture, 1350, 844, 547, 321);	
	HitNotifyIcon[EFightingVRHudPosition::BackRight] = UCanvas::MakeIcon(HitNotifyTexture, 1232, 1241, 458, 341);	
	HitNotifyIcon[EFightingVRHudPosition::Back] = UCanvas::MakeIcon(HitNotifyTexture,  862, 1384, 353, 408);	
	HitNotifyIcon[EFightingVRHudPosition::BackLeft] = UCanvas::MakeIcon(HitNotifyTexture, 454, 1251, 371, 410);	
	...
	Crosshair[EFightingVRCrosshairDirection::Left] = UCanvas::MakeIcon(HUDMainTexture, 43, 402, 25, 9); // left
	Crosshair[EFightingVRCrosshairDirection::Right] = UCanvas::MakeIcon(HUDMainTexture, 88, 402, 25, 9); // right
	Crosshair[EFightingVRCrosshairDirection::Top] = UCanvas::MakeIcon(HUDMainTexture, 74, 371, 9, 25); // top
	Crosshair[EFightingVRCrosshairDirection::Bottom] = UCanvas::MakeIcon(HUDMainTexture, 74, 415, 9, 25); // bottom
	Crosshair[EFightingVRCrosshairDirection::Center] = UCanvas::MakeIcon(HUDMainTexture, 75, 403, 7, 7); // center

	Offsets[EFightingVRHudPosition::Left] = FVector2D(173,0);	
	Offsets[EFightingVRHudPosition::FrontLeft] = FVector2D(120,125);	
	Offsets[EFightingVRHudPosition::Front] = FVector2D(0,173);	
	Offsets[EFightingVRHudPosition::FrontRight] = FVector2D(-120,125);
	Offsets[EFightingVRHudPosition::Right] = FVector2D(-173,0);	
	Offsets[EFightingVRHudPosition::BackRight] = FVector2D(-120,-125);
	Offsets[EFightingVRHudPosition::Back] = FVector2D(0,-173);
	Offsets[EFightingVRHudPosition::BackLeft] = FVector2D(120,-125);
...
void AFightingVRHUD::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	ConditionalCloseScoreboard(true);

	AFightingVRPlayerController* FightingVRPC = Cast<AFightingVRPlayerController>(PlayerOwner);
	if (FightingVRPC != NULL )
	{
		// Reset the ignore input flags, so we can control the camera during warmup
		FightingVRPC->SetCinematicMode(false,false,false,true,true);
	}

	Super::EndPlay(EndPlayReason);
}

void AFightingVRHUD::SetMatchState(EFightingVRMatchState::Type NewState)
{
	MatchState = NewState;
}

EFightingVRMatchState::Type AFightingVRHUD::GetMatchState() const
{
	return MatchState;
}

FString AFightingVRHUD::GetTimeString(float TimeSeconds)
{
...
void AFightingVRHUD::DrawWeaponHUD()
{
	AFightingVRCharacter* MyPawn = CastChecked<AFightingVRCharacter>(GetOwningPawn());
	AFightingVRWeapon* MyWeapon = MyPawn->GetWeapon();
	if (MyWeapon)
	{
		...
		//SECONDARY WEAPON
		AFightingVRWeapon* SecondaryWeapon = NULL;
...
void AFightingVRHUD::DrawHealth()
{
	AFightingVRCharacter* MyPawn = Cast<AFightingVRCharacter>(GetOwningPawn());
	Canvas->SetDrawColor(FColor::White);
...
void AFightingVRHUD::DrawMatchTimerAndPosition()
{
	...
	if (MyGameState && MatchState == EFightingVRMatchState::Playing)
	{
	...
	if (MyGameState && MyGameState->RemainingTime > 0)
	{
		...
		Text = FString();
		AFightingVRPlayerController* MyPC = Cast<AFightingVRPlayerController>(PlayerOwner);
		if (MyPC && MyGameState && MatchState == EFightingVRMatchState::Playing)
		{
...
void AFightingVRHUD::NotifyOutOfAmmo()
{
	NoAmmoNotifyTime = GetWorld()->GetTimeSeconds();
}

void AFightingVRHUD::NotifyEnemyHit()
{
	LastEnemyHitTime = GetWorld()->GetTimeSeconds();
}

void AFightingVRHUD::DrawHUD()
{
	Super::DrawHUD();
	if (Canvas == nullptr)
	{
...
void AFightingVRHUD::DrawDebugInfoString(const FString& Text, float PosX, float PosY, bool bAlignLeft, bool bAlignTop, const FColor& TextColor)
{
...
void AFightingVRHUD::DrawCrosshair()
{
	AFightingVRPlayerController* PCOwner = Cast<AFightingVRPlayerController>(PlayerOwner);
	if (PCOwner)
	{
		AFightingVRCharacter* Pawn =  Cast<AFightingVRCharacter>(PCOwner->GetPawn());
			...
			&& (Pawn->IsTargeting() || (!Pawn->IsTargeting() && !Pawn->GetWeapon()->bHideCrosshairWhileNotAiming)))
		{
			...
			AFightingVRWeapon_Instant* InstantWeapon = Cast<AFightingVRWeapon_Instant>(Pawn->GetWeapon());
			AFightingVRWeapon* MyWeapon = Pawn->GetWeapon();
			const float CurrentTime = GetWorld()->GetTimeSeconds();
			...
			if (Pawn->IsTargeting() && MyWeapon && MyWeapon->UseLaserDot)
			{
				...
				Canvas->DrawIcon(*CurrentCrosshair[EFightingVRCrosshairDirection::Center],
					CenterX - (*CurrentCrosshair[EFightingVRCrosshairDirection::Center]).UL*ScaleUI / 2.0f,
					CenterY - (*CurrentCrosshair[EFightingVRCrosshairDirection::Center]).VL*ScaleUI / 2.0f, ScaleUI);
			}
			else
			{
				Canvas->DrawIcon(*CurrentCrosshair[EFightingVRCrosshairDirection::Center], 
				...
				Canvas->DrawIcon(*CurrentCrosshair[EFightingVRCrosshairDirection::Left],
					CenterX - 1 - (*CurrentCrosshair[EFightingVRCrosshairDirection::Left]).UL * ScaleUI - CrossSpread * ScaleUI, 
					CenterY - (*CurrentCrosshair[EFightingVRCrosshairDirection::Left]).VL*ScaleUI / 2.0f, ScaleUI);
				Canvas->DrawIcon(*CurrentCrosshair[EFightingVRCrosshairDirection::Right], 
					CenterX + CrossSpread * ScaleUI, 
					CenterY - (*CurrentCrosshair[EFightingVRCrosshairDirection::Right]).VL * ScaleUI / 2.0f, ScaleUI);

				Canvas->DrawIcon(*CurrentCrosshair[EFightingVRCrosshairDirection::Top], 
					CenterX - (*CurrentCrosshair[EFightingVRCrosshairDirection::Top]).UL * ScaleUI / 2.0f,
					CenterY - 1 - (*CurrentCrosshair[EFightingVRCrosshairDirection::Top]).VL * ScaleUI - CrossSpread * ScaleUI, ScaleUI);
				Canvas->DrawIcon(*CurrentCrosshair[EFightingVRCrosshairDirection::Bottom],
					CenterX - (*CurrentCrosshair[EFightingVRCrosshairDirection::Bottom]).UL * ScaleUI / 2.0f,
...
void AFightingVRHUD::DrawDeathMessages()
{
	if (PlayerOwner == NULL)
	{
		return;
...
void AFightingVRHUD::ShowDeathMessage(class AFightingVRPlayerState* KillerPlayerState, class AFightingVRPlayerState* VictimPlayerState, const UDamageType* KillerDamageType)
{
	...
	if (GetWorld()->GetGameState())
	{
		...
		AFightingVRPlayerState* MyPlayerState = PlayerOwner ? Cast<AFightingVRPlayerState>(PlayerOwner->PlayerState) : NULL;
		...
		if (DefGame && KillerPlayerState && VictimPlayerState && MyPlayerState)
		{
			...
			NewMessage.DamageType = MakeWeakObjectPtr(const_cast<UFightingVRDamageType*>(Cast<const UFightingVRDamageType>(KillerDamageType)));
...
void AFightingVRHUD::NotifyWeaponHit(float DamageTaken, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator)
{
	const float CurrentTime = GetWorld()->GetTimeSeconds();
	AFightingVRCharacter* MyPawn = (PlayerOwner) ? Cast<AFightingVRCharacter>(PlayerOwner->GetPawn()) : NULL;
	if (MyPawn)
	{
...
void AFightingVRHUD::DrawHitIndicator()
{
	const float CurrentTime = GetWorld()->GetTimeSeconds();
	if (CurrentTime - LastHitTime <= HitNotifyDisplayTime)
	{
...
void AFightingVRHUD::OnPlayerTalkingStateChanged(TSharedRef<const FUniqueNetId> TalkingPlayerId, bool bIsTalking)
{
...
void AFightingVRHUD::PostInitializeComponents()
{
...
void AFightingVRHUD::ConditionalCloseScoreboard(bool bFocus)
{
	if (bIsScoreBoardVisible)
	{
...
void AFightingVRHUD::ToggleScoreboard()
{
	ShowScoreboard(!bIsScoreBoardVisible);
}

bool AFightingVRHUD::ShowScoreboard(bool bEnable, bool bFocus)
{
	...
	{
		AFightingVRPlayerController* FightingVRPC = Cast<AFightingVRPlayerController>(PlayerOwner);
		if (FightingVRPC == NULL || FightingVRPC->IsGameMenuVisible() )
		{
			return false;
		}
	}
	...
	if (bIsScoreBoardVisible)
	{
		...
		.Padding(FMargin(50))
		[
			SAssignNew(ScoreboardWidget, SFightingVRScoreboardWidget)
...
void AFightingVRHUD::ToggleChat()
{
	AFightingVRPlayerController* FightingVRPC = Cast<AFightingVRPlayerController>(PlayerOwner);
	// If the game menu is visible dont show the chat menu
	if( (FightingVRPC == NULL ) || FightingVRPC->IsGameMenuVisible() || GetMatchState() == EFightingVRMatchState::Warmup )
	{
		return;
	}
...
void AFightingVRHUD::SetChatVisibilty( const EVisibility RequiredVisibility )
{
	TryCreateChatWidget();
...
void AFightingVRHUD::AddChatLine(const FText& ChatString, bool bWantFocus)
{
	TryCreateChatWidget();
...
void AFightingVRHUD::MakeUV(FCanvasIcon& Icon, FVector2D& UV0, FVector2D& UV1, uint16 U, uint16 V, uint16 UL, uint16 VL)
{
	if (Icon.Texture)
	{
...
bool AFightingVRHUD::TryCreateChatWidget()
{
	bool bCreated = false;
	AFightingVRPlayerController* FightingVRPC = Cast<AFightingVRPlayerController>(PlayerOwner);
	if(FightingVRPC == NULL )
	{
		UE_LOG(LogFightingVR, Warning, TEXT("Unable to create chat widget - Invalid player controller") );
	}
	else
	{
		...
		{
			bCreated = true;
			FLocalPlayerContext WorldContext(FightingVRPC);
			GEngine->GameViewport->AddViewportWidgetContent(
...
bool AFightingVRHUD::IsMatchOver() const
{
	return GetMatchState() == EFightingVRMatchState::Lost || GetMatchState() == EFightingVRMatchState::Won;
}

void AFightingVRHUD::AddMatchInfoString(const FCanvasTextItem InInfoItem )
{
	InfoItems.Add(InInfoItem);
}

float AFightingVRHUD::ShowInfoItems(float YOffset, float TextScale)
{
	float Y = YOffset;
...
float AFightingVRHUD::DrawRecentlyKilledPlayer()
{		
