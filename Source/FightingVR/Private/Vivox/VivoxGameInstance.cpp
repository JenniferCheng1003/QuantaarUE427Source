...
UVivoxGameInstance::UVivoxGameInstance(const FObjectInitializer & ObjectInitializer) : UFightingVRInstance(ObjectInitializer)
{
...
void UVivoxGameInstance::StartGameInstance()
{
    ...
    // name instead.
    // Since FightingVR does not use its own game server or have accounts, we use the
...
VivoxCoreError UVivoxGameInstance::Initialize(int logLevel)
{
    ...
    if (GIsEditor)
    {
        UE_LOG(LogVivoxGameInstance, Warning, TEXT("This sample does not support Play In-Editor: the Vivox Plugin does, but hosting or joining a match in the underlying FightingVR sample (base app, not our sample integration) doesn't function properly in the editor, so the plugin cannot adequately be showcased."));
...
#if (PLATFORM_PS4 || PLATFORM_XBOXONE || PLATFORM_SWITCH)
    if (NewState == FightingVRInstanceState::MainMenu)
    {
