...
{
    // skip the base class AFightingVRHUD::PostInitializeComponents method to avoid delegate binding that enables speaking indicators to show on the scoreboard.
...
FLinearColor AVivoxHUD::GetTeamColor()
{
    ...
    {
        AFightingVRPlayerController* MyPlayerController = Cast<AFightingVRPlayerController>(PlayerOwner);
        if (MyPlayerController)
        {
            AFightingVRPlayerState* MyPlayerState = Cast<AFightingVRPlayerState>(MyPlayerController->PlayerState);
            if (MyPlayerState)
            {
...
void AVivoxHUD::DrawVivoxInfoText()
{
    ...
    if (MatchState == EFightingVRMatchState::Warmup || (MatchState == EFightingVRMatchState::Playing && !(MyPawn && MyPawn->IsAlive())))
    {
...
void AVivoxHUD::DrawVivoxRoster()
{
    ...
    TArray<AFightingVRPlayerState*> TeamArray;
    for (int32 i = 0; i < MyGameState->PlayerArray.Num(); ++i)
    {
        AFightingVRPlayerState* CurPlayerState = Cast<AFightingVRPlayerState>(MyGameState->PlayerArray[i]);
    ...
    TeamArray.Sort([this](const AFightingVRPlayerState& One, const AFightingVRPlayerState& Two) {
    ...
    {
        AFightingVRPlayerState* CurPlayerState = TeamArray[PlayerIndex];
